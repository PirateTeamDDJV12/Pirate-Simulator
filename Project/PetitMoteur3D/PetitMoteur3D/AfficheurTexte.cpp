...
namespace PM3D
{
    ...
    CAfficheurTexte::CAfficheurTexte(int largeur, int hauteur, Gdiplus::Font* pPolice, Gdiplus::Color couleurTexte, Gdiplus::Color couleurTex)
    {
        // Prendre en note le dispositif, la largeur et la hauteur
        pDispo = PirateSimulator::RendererManager::singleton.getDispositif();
        TexWidth = largeur;
        TexHeight = hauteur;
        ...
        pBlackBrush = new SolidBrush(couleurTexte);

        // On efface le bitmap (notez le NOIR TRANSPARENT...)
        pCharGraphics->Clear(couleurTex);
        ...
        hr = pDispo->GetD3DDevice()->CreateShaderResourceView(pTexture, &srvDesc, &pTextureView);
