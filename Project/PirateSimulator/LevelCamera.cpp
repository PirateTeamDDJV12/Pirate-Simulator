...
void LevelCamera::move(Move::Translation::Direction direction)
{
    ...
    auto temp = m_transform.m_position;
    temp.vector4_f32[1] = m_terrain->getHeight(m_transform.m_position) + m_offsetCam;

    m_transform.m_position = XMVectorLerp(m_transform.m_position, temp, .1f);
...
void LevelCamera::rotate(Move::Rotation::Direction direction)
{
    ...
    m_transform.m_right = XMVector3Normalize(XMVector3Cross(m_transform.m_up, m_transform.m_forward));
